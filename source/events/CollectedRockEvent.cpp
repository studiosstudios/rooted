 ...
 */
std::shared_ptr<NetEvent> CollectedRockEvent::newEvent(){
    return std::make_shared<CollectedRockEvent>();
}

std::shared_ptr<NetEvent> CollectedRockEvent::allocCollectedRockEvent(std::string uuid, int rockid){
    //TODO: make a new shared copy of the event and set its _pos to pos.
#pragma mark BEGIN SOLUTION
    auto event = std::make_shared<CollectedRockEvent>();
    event->_uuid = uuid;
    event->_rockID = rockid;
    return event;
 ...
 */
std::vector<std::byte> CollectedRockEvent::serialize(){
    //TODO: serialize _pos
#pragma mark BEGIN SOLUTION
    _serializer.reset();
    _serializer.writeString(_uuid);
    _serializer.writeSint32(_rockID);
 ...
 */
void CollectedRockEvent::deserialize(const std::vector<std::byte>& data){
...
#pragma mark BEGIN SOLUTION
    ...
    _uuid = _deserializer.readString();
    _rockID = _deserializer.readSint32();
#pragma mark END SOLUTION
}
