...
build() {
    echo "Building edu.cornell.gdiac.rooted.flatpak"

    # Create the flatpak
    flatpak-builder --repo=repo --force-clean cache ${BASEDIR}/edu.cornell.gdiac.rooted.yml
    flatpak build-bundle repo ${BASEDIR}/edu.cornell.gdiac.rooted.flatpak edu.cornell.gdiac.rooted
    rm -rf ${INSTDIR}
    mkdir ${INSTDIR}
    mv ${BASEDIR}/edu.cornell.gdiac.rooted.flatpak ${INSTDIR}/

    # Create the Steam shortcut
    echo "#!/bin/sh" >> "${INSTDIR}/rooted.exe"
    echo "flatpak run edu.cornell.gdiac.rooted" >> "${INSTDIR}/rooted.exe"
    chmod a+x "${INSTDIR}/rooted.exe"

    # Create the flatpak installer
    echo "#!/bin/sh" >> ${INSTDIR}/install.sh
    echo "BASEDIR=\$(dirname \$0)" >> ${INSTDIR}/install.sh
    echo "flatpak install \${BASEDIR}/edu.cornell.gdiac.rooted.flatpak " >> ${INSTDIR}/install.sh
