...
void UIController::initJoystickNodes() {
    ...
    _joyback = scene2::PolygonNode::allocWithPoly(_pf.makeCircle(Vec2(0,0), _drawScale * 2));
    _joyback->setColor(Color4(0, 0, 0, 100));
    _joynode->addChild(_joyback);
    
    _joymain = scene2::PolygonNode::allocWithPoly(_pf.makeCircle(Vec2(0,0), _drawScale));
...
bool UIController::init(const std::shared_ptr<cugl::scene2::SceneNode> uinode,
    ...
    _pf = PolyFactory(0.5 * drawScale / DEFAULT_DRAWSCALE);
