...
//
//  This module provides one-stop shopping for basic 2d graphics in OpenGL.  Despite 
//  the name, it is also capable of drawing solid shapes, as well as wireframes.
...
bool SpriteBatch::init(unsigned int capacity, const std::shared_ptr<Shader>& shader) {
    ...
    // TODO: Refactor this into ShaderData
...
void SpriteBatch::end() {
    ...
    cugl::stencil::applyEffect(StencilEffect::NONE, _shader);
...
void SpriteBatch::flush() {
    ...
    for(auto it = _history.begin(); it != _history.end(); ++it) {
        ...
        if (next->dirty & DIRTY_BLENDEQUATION) {
            _shader->setBlendEquation(next->blendEq);
        }
        if (next->dirty & DIRTY_SRC_FUNCTION || next->dirty & DIRTY_DST_FUNCTION) {
            if (next->srcRGB != next->srcAlpha || next->dstRGB != next->dstAlpha ) {
                _shader->setBlendFuncSeperate(next->srcRGB, next->dstRGB,
                                              next->srcAlpha, next->dstAlpha);
            } else {
                _shader->setBlendFunc(next->srcRGB, next->dstRGB);
            }
        }
        ...
        if (next->dirty & DIRTY_STENCIL_CLEAR) {
            cugl::stencil::clearBuffer(next->cleared);
        }
 ...
 */
void SpriteBatch::drawMesh(const Mesh<SpriteVertex2>& mesh, const Vec2 offset,
                           bool tint) {
 ...
 * @param tint      Whether to tint with the active color
 */
void SpriteBatch::drawMesh(const Mesh<SpriteVertex2>& mesh,
 ...
 * @param tint      Whether to tint with the active color
 */
void SpriteBatch::drawMesh(const SpriteVertex2* vertices, size_t size,
    ...
    for(int ii = 0;  ii < indices->size(); ii += chunksize) {
        ...
        for(int jj = 0; jj < chunksize; jj++) {
            ...
            if (search != offsets.end()) {
                ...
                _vertData[_vertSize].gradcoord.x = point.x;
                _vertData[_vertSize].gradcoord.y = point.y;
