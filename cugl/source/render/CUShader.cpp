 ...
 */
static std::string preprocess_shader(const std::string code) {
 ...
 *
 * If culling is being enabled, the user must set the cull mode
 * after this call. This is because Vulkan stores culling being enabled
 * or disabled as a state in the cull mode rather than a seperate value,
 * which means disabling culling loses whatever state was stored previously.
 *
 * @param enable    Whether to enable culling
 */
void Shader::enableCulling(bool enable) {
    if (enable) {
 ...
 */
void Shader::setCullMode(GLenum mode) {
    glCullFace(mode);
}
 ...
 *
 * @param enable    Whether to enable or disable the depth test
 */
 ...
 *
 * For Vulkan, Depth writing will only work if the depth test is also enabled (but 
 * it can be set to always succeed to still ignore it)
 *
 * @param enable    Whether depth writing is enabled or not
 */
 ...
 *
 * @param compareOp The new depth compare operation
 */
void Shader::setDepthCompareOp(GLenum compareOp) {
 ...
 */
void Shader::setBlendFunc(GLenum srcFactor, GLenum dstFactor) {
 ...
 * Sets the blend function for this pipeline
 *
 * @param srcRGB    The source factor
 * @param dstRGB    The destination factor
 * @param srcAlpha  The source alpha factor
 * @param dstAlpha  The destination alpha factor
 */
void Shader::setBlendFuncSeperate(GLenum srcRGB, GLenum dstRGB,
 ...
 *
 * @param op    The blend equation
 */
void Shader::setBlendEquation(GLenum op) {
    glBlendEquation(op);
}
 ...
 * Sets the blend equation for this pipeline
 *
 * @param opRGB     The RGB blend equation
 * @param opAlpha   The alpha blend equation
 */
void Shader::setBlendEquationSeperate(GLenum opRGB, GLenum opAlpha) {
 ...
 */
void Shader::enableBlending(bool enable) {
    if (enable) {
        glEnable(GL_BLEND);
    } else {
        glDisable(GL_BLEND);
...
/**
 * Enables the stencil for this pipeline
 *
 * @param enable    Whether to enable stencil
 */
void Shader::enableStencil(bool enable) {
    if (enable) {
 ...
 *
 * @param fail      The operation to perform when the stencil test fails
 * @param dpfail    The operation to perform when the stencil test passes, but the depth test fails
 * @param pass      The operation to perform when both the stencil and depth tests pass
 */
void Shader::setStencilOp(GLenum fail, GLenum dpfail, GLenum pass) {
    glStencilOp(fail, dpfail, pass);
}
 ...
 * @param pass      The operation to perform when both the stencil and depth tests pass
 */
void Shader::setStencilOpFront(GLenum fail, GLenum dpfail, GLenum pass) {
    glStencilOpSeparate(GL_FRONT, fail, dpfail, pass);
}
 ...
 * @param pass      The operation to perform when both the stencil and depth tests pass
 */
void Shader::setStencilOpBack(GLenum fail, GLenum dpfail, GLenum pass) {
    glStencilOpSeparate(GL_BACK, fail, dpfail, pass);
}

/**
 * Sets the stencil compare operation for this pipeline
 *
 * @param compareOp     The stencil compare operation
 * @param ref           The reference value for the stencil test
 * @param compareMask   The mask to apply to both the reference value and stored stencil value
 */
void Shader::setStencilFunc(GLenum compareOp, uint32_t ref, uint32_t compareMask) {
    glStencilFunc(compareOp, ref, compareMask);
}

/**
 * Sets the stencil write mask for this pipeline
 *
 * @param writeMask The write mask to apply to values written to the stencil buffer
 */
void Shader::setStencilMask(uint32_t writeMask) {
 ...
 */
void Shader::setColorMask(GLboolean r, GLboolean g, GLboolean b, GLboolean a) {
    glColorMask(r, g, b, a);
}
