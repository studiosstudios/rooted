...
bool GroundRenderer::init(const std::shared_ptr<cugl::AssetManager> &assets, const std::shared_ptr<cugl::OrthographicCamera> &camera,
...
void GroundRenderer::dispose() {
    _shader = nullptr;
    _vertbuff = nullptr;
}

void GroundRenderer::update(float timestep) {
    if (_shader) {
        _shader->bind();
        _totalTime += timestep;
        if (_totalTime >= 30.0) {
...
void GroundRenderer::buildShader() {
