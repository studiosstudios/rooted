 ...
 */
std::shared_ptr<NetEvent> SpawnRockEvent::newEvent(){
    return std::make_shared<SpawnRockEvent>();
}

std::shared_ptr<NetEvent> SpawnRockEvent::allocSpawnRockEvent(Vec2 pos, int idx, Vec2 vel, std::string uuid){
 ...
 */
std::vector<std::byte> SpawnRockEvent::serialize(){
    //TODO: serialize _pos
#pragma mark BEGIN SOLUTION
    _serializer.reset();
 ...
 */
void SpawnRockEvent::deserialize(const std::vector<std::byte>& data){
